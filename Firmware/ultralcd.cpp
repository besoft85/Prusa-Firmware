...
void lcdui_print_status_line(void) {
    ...
    if (heating_status != HeatingStatus::NO_HEATING) { // If heating flag, show progress of heating
        ...
        lcd_set_cursor(7, (LCD_HEIGHT - 1));/*RAMPS*/
        lcd_space(13);

        for (uint8_t dots = 0; dots < heating_status_counter; dots++) {
            lcd_putc_at(7 + dots, (LCD_HEIGHT - 1), '.');/*RAMPS*/
        }
        switch (heating_status) {
        case HeatingStatus::EXTRUDER_HEATING:
            lcd_puts_at_P(0, (LCD_HEIGHT - 1), _T(MSG_HEATING));/*RAMPS*/
            break;
        case HeatingStatus::EXTRUDER_HEATING_COMPLETE:
            lcd_puts_at_P(0, (LCD_HEIGHT - 1), _T(MSG_HEATING_COMPLETE));/*RAMPS*/
            heating_status = HeatingStatus::NO_HEATING;
            heating_status_counter = 0;
            break;
        case HeatingStatus::BED_HEATING:
            lcd_puts_at_P(0, (LCD_HEIGHT - 1), _T(MSG_BED_HEATING));/*RAMPS*/
            break;
        case HeatingStatus::BED_HEATING_COMPLETE:
            lcd_puts_at_P(0, (LCD_HEIGHT - 1), _T(MSG_BED_DONE));/*RAMPS*/
            heating_status = HeatingStatus::NO_HEATING;
            heating_status_counter = 0;
            break;
        default:
        ...
        lcd_status_message_timeout.expired_cont(LCD_STATUS_INFO_TIMEOUT))
    {
        ...
        {
            lcd_set_cursor(lcd_status_message_idx, (LCD_HEIGHT - 1));/*RAMPS*/
        ...
        case CustomMsg::MeshBedLeveling: // If mesh bed leveling in progress, show the status
            if (custom_message_state > 10) {
                lcd_set_cursor(0, (LCD_HEIGHT - 1));/*RAMPS*/
                lcd_space(LCD_WIDTH);
                lcd_puts_at_P(0, (LCD_HEIGHT - 1), _T(MSG_CALIBRATE_Z_AUTO));/*RAMPS*/
                lcd_puts_P(PSTR(" : "));
                ...
                if (custom_message_state > 3 && custom_message_state <= 10) {
                    lcd_set_cursor(0, (LCD_HEIGHT - 1));/*RAMPS*/
                    lcd_space(19);
                    lcd_puts_at_P(0, (LCD_HEIGHT - 1), _T(MSG_HOMEYZ_DONE)); ////MSG_HOMEYZ_DONE c=20 /*RAMPS*/
        ...
        case CustomMsg::PidCal: // PID tuning in progress
            ...
            if (pid_cycle <= pid_number_of_cycles && custom_message_state > 0) {
                lcd_set_cursor(10, (LCD_HEIGHT - 1));/*RAMPS*/
        ...
        case CustomMsg::TempCompPreheat:              // temp compensation preheat
            lcd_puts_at_P(0, (LCD_HEIGHT - 1), _T(MSG_PINDA_PREHEAT));/*RAMPS*/
        ...
        case CustomMsg::Resuming: // Resuming
            lcd_puts_at_P(0, (LCD_HEIGHT - 1), _T(MSG_RESUMING_PRINT));/*RAMPS*/
            break;
        }
    }
}
...
static void lcd_menu_temperatures_line(const char *ipgmLabel, int value){
    ...
    lcd_printf_P(PSTR(" %s%3d" LCD_STR_DEGREE " \n"), tmp, value); // no need to add -14.14 to string alignment /*RAMPS*/
}
...
static void lcd_loading_progress_bar(uint16_t loading_time_ms) {
	...
	for (uint_least8_t i = 0; i < LCD_WIDTH; i++) {
		lcd_putc_at(i, (LCD_HEIGHT - 1), '.');/*RAMPS*/
...
static void lcd_menu_xyz_skew()
{
	...
	lcd_set_cursor(15, 0);
	if (angleDiff < 100){
		lcd_printf_P(_N("%3.2f" LCD_STR_DEGREE), _deg(angleDiff));/*RAMPS*/
	} else {
...
void lcd_show_fullscreen_message_and_wait_P(const char *msg)
{
	...
	for (;;) {
		if (msg_next == NULL) {
			...
			lcd_putc_at(19, (LCD_HEIGHT - 1), LCD_STR_CONFIRM[0]);/*RAMPS*/
		}
...
{
    lcd_putc_at(0, (LCD_HEIGHT - 1), selected == LCD_LEFT_BUTTON_CHOICE ? '>': ' '); /*RAMPS*/
    lcd_puts_P(first_choice);
    lcd_putc_at(second_col, (LCD_HEIGHT - 1), selected == LCD_MIDDLE_BUTTON_CHOICE ? '>': ' '); /*RAMPS*/
    lcd_puts_P(second_choice);
    if (third_choice) {
        lcd_putc_at(18, (LCD_HEIGHT - 1), selected == LCD_RIGHT_BUTTON_CHOICE ? '>': ' '); /*RAMPS*/
...
void lcd_belttest()
{
    ...
    if (lcd_selfcheck_axis_sg(X_AXIS)){
		...
		if (lcd_selfcheck_axis_sg(Y_AXIS))
		{
			...
			lcd_set_cursor(10, 3);
			lcd_print(Y);
			lcd_putc_at(19, (LCD_HEIGHT - 1), LCD_STR_UPLEVEL[0]);/*RAMPS*/
